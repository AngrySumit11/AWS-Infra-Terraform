...
resource "aws_cloudwatch_metric_alarm" "unhealthy-host-count" {
  ...
  alarm_name          = format("%s-unhealthy-host-count-%s", local.load_balancer, var.region_short)
  ...
  tags = {
    "Name" = format("%s-unhealthy-host-count-%s", local.load_balancer, var.region_short)
  }
}
...
resource "aws_cloudwatch_metric_alarm" "http-code-4xx-count" {
  ...
  alarm_name          = format("%s-http-code-4xx-count-%s", local.load_balancer, var.region_short)
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
  ...
  tags = {
    "Name" = format("%s-http-code-4xx-count-%s", local.load_balancer, var.region_short)
  }
}
...
resource "aws_cloudwatch_metric_alarm" "http-code-5xx-count" {
  ...
  alarm_name          = format("%s-http-code-5xx-count-%s", local.load_balancer, var.region_short)
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
  ...
  tags = {
    "Name" = format("%s-http-code-5xx-count-%s", local.load_balancer, var.region_short)
  }
}
