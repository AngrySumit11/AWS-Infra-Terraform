resource "aws_rds_cluster" "course1" {
  ...
  db_subnet_group_name            = var.environment == "prod" || var.environment == "dev" || var.environment == "test" ? aws_db_subnet_group.db_subnet_group.id : "db-subnets-${var.environment}"
  ...
  depends_on = [aws_db_parameter_group.course1, aws_db_subnet_group.db_subnet_group]
}
...
resource "aws_db_subnet_group" "db_subnet_group" {
  ...
  tags = {
    "Name" = "${var.subnet_prefix}-${var.environment}"
  }
}
...
resource "aws_rds_cluster" "rds_cluster_encryption" {
  ...
  depends_on = [aws_db_parameter_group.db_parameter_group_encryption, aws_db_subnet_group.db_subnet_group_encryption]
}
...
resource "aws_db_subnet_group" "db_subnet_group_encryption" {
  name       = "${var.subnet_prefix}-${var.primary_name}-${var.environment}-encryption"
  subnet_ids = var.subnet_ids_list
