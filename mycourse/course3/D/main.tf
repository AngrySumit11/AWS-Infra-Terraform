...
resource "aws_route53_record" "course1-external-lb" {
  ...
  type    = "CNAME"
  ttl     = "300"
  records = [var.course1_external_lb_dns]
}
...
resource "aws_route53_record" "course1-internal-lb" {
  ...
  type           = "CNAME"
  ttl            = "60"
  set_identifier = "${var.environment}-${var.region_short}"
  geolocation_routing_policy {
    country = "*"
  }
...
resource "aws_route53_record" "external-web-lb-hub" {
  provider = aws.course1-non-prod-us-west-2

  zone_id = var.public_domain_zone_id
  name    = var.external_web_lb_hub
  type    = "CNAME"
  ttl     = "60"
  records = [var.external_web_lb_value_hub]
}

*/
resource "aws_route53_record" "course1-external-lb-hub" {
  provider = aws.course1-non-prod-us-west-2

  zone_id = var.public_domain_zone_id
  name    = var.course1_external_lb_dns_value_hub
  type    = "CNAME"
  ttl     = "300"
  records = [var.course1_external_lb_dns_hub]
}
...
/*
resource "aws_route53_record" "meet_app_external_lb" {
  provider = aws.course1-non-prod-us-west-2

  zone_id        = var.public_domain_zone_id
  name           = var.meet_app_external_lb_dns
  type           = "CNAME"
  ttl            = "60"
  records = [var.meet_app_external_lb_dns_value]
}
*/